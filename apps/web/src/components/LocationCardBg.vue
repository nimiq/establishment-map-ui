...
const banner = computed(() => {
  if (props.banner) return props.banner
  if (Array.isArray(props.location.banner)) return props.location.banner[0]
...
<template>
  <template v-if="!location.isAtm">
    <div v-if="withGradient" rounded-bl="[var(bottom-radius-bl,var(bottom-radius))]"
    ...
    <div v-bind="$attrs" pointer-events-none absolute bottom-0 right-0 z-20 h-full w-full of-hidden>
      <div v-if="location.splitBanner" i-ring:provider-small absolute top-8 right-8 size-48 />
      <div v-else i-ring:provider absolute bottom--62 right--62 size-188 />
      <div absolute bottom-16 right-16 size-32 rounded-full :class="{
