...
<template>
  <transition leave-active-class="duration-150 ease-in" leave-from-class="opacity-100" leave-to-class="opacity-0">
    <div v-if="showSplashScreen" absolute inset-0 grid h-screen h-100dvh px-48 z-100 place-content-center bg-neutral-0>
      <div i-nimiq:logos-crypto-map-horizontal dark:i-nimiq:logos-crypto-map-white-horizontal aria-hidden
        class="!h-64 !w-[min(510px,90vw)] animate-fade" />
      <i18n-t absolute inset-x-0 bottom-0 w-screen p-16 text="12 desktop:center neutral-800" tag="div"
        ...
        <template #privacyPolicy>
          <a href="https://policies.google.com/privacy" target="_blank" rel="noopener"
            class="underline text-space/80 dark:text-white/60">{{ $t('Privacy Policy') }}</a>
        </template>
        <template #termsOfService>
          <a href="https://policies.google.com/terms" target="_blank" rel="noopener"
            class="underline text-space/80 dark:text-white/60">{{ $t('Terms of Service') }}</a>
        </template>
      </i18n-t>
    </div>
  </transition>
