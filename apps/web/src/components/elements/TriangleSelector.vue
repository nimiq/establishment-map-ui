  ...
  <SelectRoot v-model="selected">
    ...
    <SelectPortal>
      <SelectContent bg-gradient-neutral text-neutral-0 shadow rounded-6 bottom-0 drop-shadow relative side="bottom" position="item-aligned" py-6 animate-fade-in animate-duration-100 z-100>
        <SelectViewport>
          <SelectItem v-for="option in options" :key="option" :value="option" px-12 py-4 cursor-pointer group  outline="none hover:none focus-visible:blue">
            <SelectItemText label text="14 neutral-600/80 group-hover:neutral-0" transition-colors flex="~ items-center gap-8">
              {{ option }}
