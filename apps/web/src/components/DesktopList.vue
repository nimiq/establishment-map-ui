...
<template>
    ...
    <template #default="{ item: location, active }">
      <DynamicScrollerItem :key="location.uuid" :item="location" :active="active">
        <button w-full px-24 py-20 text-left bg="$bg-1 image-$bg-1 hocus:$bg-2 hocus:image-$bg-2" transition-colors
          :data-inverted="location.isAtm && location.cardStyle.isDark ? 'true' : undefined" :style="{
            '--bg-1': location.isAtm && location.cardStyle.isDark ? location.style.bg[0] : 'rgb(var(--nq-neutral-0))',
            '--bg-2': location.isAtm && location.cardStyle.isDark && location.style.bg[1] ? location.style.bg[1] : 'rgb(var(--nq-neutral-100))',
  ...
  <div v-else flex="~ col items-center justify-center gap-24" max-w-320 of-hidden px-20 transition-height duration-300
    will-change-height
    :style="{ height: listIsShown ? 'calc(100vh - 10.5rem - var(--dynamic-block, 0) * 88px)' : '0' }">
