...
<template>
  <CustomMarker v-for="c in clusters" :key="c.id"
    ...
    <OnClickOutside @trigger="isMobile && toggleClusterExpansion(c, $event)">
      <ul :data-cluster-id="c.id" relative :style="getClusterStyles(c)" @pointerover="e => onPointer(e, c)"
        @pointerout="e => onPointer(e, c)">
        <li relative z-10>
          <button grid="~ place-content-center" text="14 neutral-0 dark:white/80"
            bg="neutral dark:blue hover:neutral-900" ring="1.5 neutral/10" transition-colors font-bold rounded-full
            shadow cursor-pointer aspect-square
