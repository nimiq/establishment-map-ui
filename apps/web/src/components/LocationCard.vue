...
<template>
  <div duration="$duration,0" :class="{
    ...
    <div v-if="location.photo && progress > 0" px-5 pt-6 transition-height duration="[--duration]"
      ...
      <img h-full w-full rounded-8 object-cover bg="neutral/10 inverted:neutral-0/60" :src="location.photo"
    ...
    <div relative px-24 py-20>
      ...
      <div v-if="location.cryptocity" ring="1 neutral-200 inverted:white/40" flex="~ gap-x-8" mb-20 mt-6 rounded-8 px-12
    ...
    <Banner v-if="progress > 0 && location.banner" :location absolute mt--36 w-full
