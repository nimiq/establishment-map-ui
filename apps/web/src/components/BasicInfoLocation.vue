...
<template>
  <div :grid="`~ rows-[repeat(3,auto)] gap-x-6 ${location.photo ? 'cols-[auto_1fr]' : 'cols-[auto_1fr_auto]'}`">
    ...
    <LocationExternalUrl v-if="!location.photo && location.url && progress > 0.5" :location mr-20 self-start
    ...
    <template v-if="!location.isAtm">
      ...
      <div v-if="location.rating" row-start-2 flex="~ gap-x-2 self-center" text-12>
        <div v-for="i in 5" :key="i" i-nimiq:star
    ...
    <p v-if="location.address" row-start-3 leading-18 :class="location.photo ? 'col-span-2' : 'col-span-3'"
