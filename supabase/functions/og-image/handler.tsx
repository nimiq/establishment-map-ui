...
const fetchFont = (name: string) => fetch(`${supabaseUrl}/storage/v1/object/public/images/${name}`).then(res => res.arrayBuffer())

export default async function handler() {
  const stats = await getStats({ apikey, url: supabaseUrl })
  // console.log({ stats })
  return new ImageResponse(
    <div tw="flex flex-col items-start justify-between bg-white h-full w-full text-6xl p-24" style={{ fontFamily: 'Mulish-Regular' }}>
      <div tw="flex flex-row justify-between items-center w-full">
        <div tw="flex flex-col">
          ...
          <span tw="text-[#1F2348]/60 text-[32px]">Places to Buy with Crypto and ATMs Worldwide</span>
        </div>
      ...
      </div>,
    {
      ...
      fonts: [
        {
          name: 'Mulish-Regular',
          data: await fetchFont('Mulish-Regular.ttf'),
          style: 'normal',
        },
        {
          name: 'Mulish-Semibold',
          data: await fetchFont('Mulish-SemiBold.ttf'),
          style: 'normal',
        },
        {
          name: 'Mulish-Bold',
          data: await fetchFont('Mulish-Bold.ttf'),
          style: 'normal',
        },
      ],
      headers: {
