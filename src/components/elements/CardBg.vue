...
<template>
  <template v-if="location.isShop">
    ...
    />
    <div
      v-if="location.provider !== Provider.DefaultShop"
    ...
    >
      <div
        class="absolute grid p-2 rounded-full w-9 h-9 right-4 bottom-4 place-content-center"
      ...
      >
        <ProviderIcon :provider="location.provider" class="w-6 h-6 zoom-in" />
      </div>
    </div>
  </template>
