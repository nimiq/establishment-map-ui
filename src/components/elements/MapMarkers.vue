...
<template>
  <CustomMarker
    ...
    :options="{ position: center, anchorPoint: count === 1 ? 'LEFT_CENTER' : 'CENTER' }"
  >
    <div v-if="count === 1" class="flex items-center gap-x-2 max-w-[176px] z-10">
      ...
      <span class="text-base font-semibold text-space" style="text-shadow: 0px 0px 4px white;">{{ name }}</span>
    </div>
