...
<template>
  <div class="relative grid grid-cols-[auto_1fr_72px] grid-rows-[repeat(3,auto)] items-center group/card" :class="{
    'text-white': theme === Theme.FullCardDark,
    'text-space': theme !== Theme.FullCardDark,
  }">
    <h2 class="text-base font-bold leading-[1.3] col-span-2 pb-1 text-balance truncate">{{ location.name }}
    </h2>

    <div class="relative flex self-start row-span-2 ml-5 gap-x-3">
      <Button v-if="location.url" bg-color="white" :href="location.url" border-color="grey"
        class="transition-[top,right] !w-[52px] right-0 lg:opacity-0 lg:group-hover/card:opacity-100" size="sm" :style="{
          top: location.image ? `-${progress * 184}px` : '',
          position: location.image ? `absolute` : ''
        }">
        <template #icon>
          <GmapsPin />
        </template>
      ...
      <transition enter-active-class="right-0 transition duration-100 ease-out" enter-from-class="scale-95 opacity-0"
        ...
        <Button bg-color="transparent" size="sm" class="lg:hidden invisible ml-auto !p-0 !w-1 !h-5 clickable" :class="{
          'visible': progress > 0,
    ...
    <template v-if="layout === CardLayout.Location">
      <span class="row-start-2 text-xs font-semibold capitalize text-space/60">
        {{ location.gmapsType || location.category }}
      </span>
      <div class="row-start-2 flex gap-x-0.5" v-if="location.rating">
        <template v-for="i in 5" :key="i">
          <component class="w-3 h-3" :is="i <= location.rating ? StarFilledIcon : StarIcon" />
        </template>
      </div>
    </template>
