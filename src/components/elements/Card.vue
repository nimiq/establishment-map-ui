...
<template>
  <div class="relative overflow-hidden rounded-lg duration-[--duration,0] group/card" :style="bgColor">
    <CardBg v-if="location.provider !== ProviderName.Default" :layout="layout" :provider-assets="providerAssets"
      :progress="progress" />

    <div v-if="location.image" class="pt-1.5 px-1.5 transition-height duration-[--duration]"
      :style="`height: ${progress * 184}px;`">
      <img class="object-cover w-full h-full rounded-sm" :src="location.image" alt="">
    </div>

    <div class="px-6 py-5 space-y-5">
      <BasicInfo :location="location" :layout="layout" :theme="providerAssets?.theme" :is-atm="isAtm"
      ...
      <transition enter-active-class="transition duration-100 ease-out"
        ...
        <template v-else>
          <CryptoList :cryptos="[...new Set(location.buy.concat(location.sell))]" class="relative z-20" />
        </template>
      </transition>
    </div>
