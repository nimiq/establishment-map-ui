...
definePageMeta({
  validate: async (route) => {
    ...
    if (!validNumber(route.lat)) return 'Invalid latitude'
    if (!validNumber(route.lng)) return 'Invalid longitude'
    if (!validNumber(route.zoom)) return 'Invalid zoom'
    
    const locationUuid = route.query.uuid
    if (locationUuid && !uuidRegex.test(locationUuid)) return 'Invalid UUID format'
  }
})
